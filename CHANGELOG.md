...
- inherit `MediaBox` and `Resources` through the page tree (§7.7.3.4): a leaf page that omits an inheritable attribute now takes the value from the nearest ancestor `/Pages` node by walking the `/Parent` chain (`resolve_inheritable_attr`), instead of defaulting `MediaBox` to A4 and treating `Resources` as empty. Documents that hang one resource dictionary (fonts / XObjects / shadings / colour spaces) or one media box on an intermediate page-tree node — a common authoring convention — now resolve their content correctly rather than rendering blank or mis-sized. The walk is depth-bounded (64) and cycle-guarded against a malformed self-referential `/Parent`. `CropBox` / `Rotate` are also inheritable per Table 30 but are not yet consumed by the reader, so only the two attributes the render path reads are wired through
