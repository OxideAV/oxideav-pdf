...
- fuzz (round 445): **bound `/FlateDecode` output** (§7.4.4). The crate's shared `flate_decompress` helper now routes through the compression backend's capped decode with a 512 MiB ceiling, so a decompression bomb — a few compressed bytes that declare gigabytes of plaintext — returns `Err` instead of growing the output buffer without bound. 512 MiB is far past any legitimate single PDF stream while denying a bomb the unbounded allocation
- fuzz (round 445): widen the cargo-fuzz suite from three targets to **five**, moving PDF into the fleet structure-aware / hostile-input pattern. The `parse` target now drives **every** catalog-level extraction walker on any input that opens as a document — annotations, actions, links, attachments, article threads, image + inline-image XObjects, optional content, logical reading order, signatures + document timestamps, PDF/A signals, hierarchy verification, XMP — so the full §12 interactive-feature tree surface (each with its own depth bound + cycle guard) is fuzzed, not just the navigation walkers. New **`filters`** target drives the §7.4 stream-decode primitives (ASCIIHex / ASCII85 / RunLength / LZW both `/EarlyChange` settings / Flate) and the §7.4.4.4 PNG/TIFF predictor post-filter directly on hostile bytes with attacker-controlled `/Colors` / `/BitsPerComponent` / `/Columns` / `/Predictor` geometry. New **`roundtrip`** target builds a valid-by-construction `VectorFrame` / `Scene` page plan from the fuzz input (some coordinates reinterpreted as raw `f32` so NaN / ±Inf / subnormal reach the writer) and drives it through every §7.5 file-structure serialiser — plain, cross-reference-stream, object-stream, linearized, incremental — feeding each output back through the reader, so the encode surface and the write→read contract are fuzzed rather than only the decode surface. Bounded ASan campaigns: `roundtrip` 42.5k execs clean, `parse` 727k execs clean (post-fix)
- honour **vertical writing mode** in text extraction (§9.7.4.3 + §9.4.4): a Type 0 font under `Identity-V` — or an embedded CMap whose `/WMode` is 1 — advances the text matrix vertically, `ty = (w1 − Tj/1000)·Tfs + Tc + Tw`, with `w1` from the CIDFont's `/W2` runs (both the `cfirst clast w1y v1x v1y` range form and the `c [w1y v1x v1y …]` array form) falling back to `/DW2[1]` (default −1000), and the horizontal scaling `Th` not applied per the §9.4.4 equations; `TJ` numeric adjustments land on the vertical coordinate. The §9.7.4.3 position vector `v` relates a glyph's rendering origins and does not enter the text-matrix update, so it is parsed past. Composite advance metrics refactored into a boxed `CidAdvance`. New `tests/vertical_text_round418.rs`
- read **article threads** (§12.4.3 Tables 160/161): new `threads` surface walking the catalogue `/Threads` array — each thread's `/I` information dictionary (`/Title` / `/Author` / `/Subject`, §14.3.3 syntax) plus its bead ring unrolled from `/F` through the circular `/N` chain into per-bead page index (`/P`, resolved to `/Pages` DFS order) and rectangle (`/R`). The walk stops at the conforming ring closure (last bead's `/N` refers back to the first), on any malformed revisit, and at a hard length cap. New `tests/threads_round418.rs`
- fuzz: the `parse` target now also drives the catalog-level extraction surfaces on any input that opens as a document — outline + named destinations (§12.3.3 / §12.3.2.3), page labels (§12.4.2), and text extraction (§9, incl. the embedded-CMap machinery) — with round-418 fixture seeds added to the corpus. The widened surface immediately caught a **stack overflow on a self-referential `/Pages` tree** (`/Kids [<self>]`): the Scene-side walker has been cycle-guarded since round 145, but the per-surface page walkers behind the outline/link/annotation page-index map, text extraction, and `image_xobjects` still recursed unboundedly. All three now carry the same visited-set + depth bound; captured crash input regression-tested in `tests/fuzz_regressions.rs`
- decode Type 0 fonts whose `/Encoding` is an **embedded CMap stream** (§9.7.5.3 Table 120 + §9.7.5.4 + §9.7.6.2): the CMap's `codespacerange` / `cidchar` / `cidrange` / `notdefchar` / `notdefrange` operators parse into a code → CID mapping; show-operand bytes segment at the codespace widths (successively longer codes per §9.7.6.2, the §9.7.6.3 partial-match rules choosing the consumed width for invalid codes, notdef → CID 0 fallback chain); and the §9.4.4 advance maps each code to its CID **before** the CID-keyed `/W` width lookup, so mixed-width encodings land following runs at the right origins (word spacing only on single-byte code 32 per §9.3.3). `/UseCMap` inheritance resolves from the Table 120 stream-dictionary entry (stream form recursively, depth-bounded; `Identity-H`/`Identity-V` synthesized; other predefined names degrade tolerantly — their character-collection data tables are not in ISO 32000), and `/WMode` is parsed. Extraction with `/ToUnicode` present is Unicode-correct as before; an embedded-CMap font *without* `/ToUnicode` now emits exactly one U+FFFD per extracted code (correct segmentation, honest lossy marker) instead of misreading the bytes as 2-byte identity codes. New `src/reader/cid_cmap.rs` + `tests/type0_embedded_cmap_round418.rs`
- read **page labels** (§12.4.2 Table 159): `page_label_ranges` surfaces the catalogue `/PageLabels` number tree's labelling ranges (start index, `/S` style, `/P` prefix — UTF-16BE BOM-sniffed, `/St` start value clamped to the spec's ≥ 1); `page_labels` / `page_label_map` synthesise the per-page label strings — decimal, upper/lower Roman (standard subtractive form), and upper/lower letters (A–Z, AA–ZZ, … per the Table 159 rule). The §12.4.2 EXAMPLE tree reproduces `i…iv, 1…3, A-8, A-9` exactly; a tree missing the required index-0 entry labels the uncovered leading pages with their 1-based decimal position (documented tolerance). New `tests/page_labels_round418.rs` (fixture qpdf-`--check`-clean)
- resolve **named destinations** (§12.3.2.3): new `named_destinations` surface enumerating both definition sources — the PDF 1.1 catalogue `/Dests` dictionary and the PDF 1.2+ `/Names → /Dests` name tree — merged with the name-tree entry winning on a shared key (the form §12.3.2.3 NOTE 3 steers producers toward), and `resolve_named_destination` resolving one name via the `/Limits`-guided tree descent without enumerating. Both §12.3.2.3 value shapes decode (a bare Table 151 array, or a dictionary with a `/D` entry per NOTE 2). The **outline** reader now fills `OutlineNode::destination` for a `/Dest` (or `/A` `/GoTo` `/D`) given as a Name / byte string, keeping the `named:<name>` text in `raw_dest`; the **Link-annotation** reader resolves a defined name to `PdfLinkTarget::Internal`, leaving `Named` only for names the document doesn't define. Named-dest display text now BOM-sniffs UTF-16BE per §7.9.2.2. New `tests/named_dest_round418.rs` (fixture qpdf-`--check`-clean)
- add a shared **§7.9.6 name-tree / §7.9.7 number-tree walker** (`reader::nametree`): `name_tree_entries` / `number_tree_entries` enumerate a tree's `(key, value)` pairs across `/Kids` levels with depth bounds, entry caps, and object-id cycle guards; `name_tree_lookup` descends by each node's `/Limits` window (byte-wise lexical comparison per §7.9.6) without enumerating, tolerating absent/malformed `/Limits` by descending unconditionally. Name-tree keys surface as raw byte strings (§7.9.6 byte-by-byte semantics); `decode_key_text` BOM-sniffs UTF-16BE per §7.9.2.2 for display. The attachments reader's ad-hoc `/EmbeddedFiles` walk now routes through the shared walker. New `tests/nametree_round418.rs`
- mark the internal public surface `#[doc(hidden)]` (object/xref walker plumbing, stream-filter implementations, content-stream operator machinery, DER/CMS/KARI/crypto primitives, writer helper modules) so semver tooling tracks only the documented API; attributes and comments only — no signatures or behaviour changed
- paint **inline images into the `Scene`** (§8.9.7): a `BI … ID … EI` triplet whose payload the crate decodes end-to-end (wrapping filters peeled, no terminal image codec) now splices a `Node::Image` mirroring the `Do` Image-XObject convention — device colour spaces, an inline `[/I base hival lookup]` Indexed array (hex or literal lookup string, Table 94 abbreviations), or a named `/Resources /ColorSpace` key, with the `/D` (`/Decode`) array honoured and an `/IM true` stencil poured with the current nonstroking colour (§8.9.6.2). The `BI` dict parser now reads number arrays (`/D [1 0]`) and skips string elements inside arrays (previously a hex lookup table inside `/CS [...]` aborted the parse and the salvage path dropped the event); an inline Indexed array is tagged `ColorSpace::Indexed` on the public `PdfInlineImage` surface (it previously fell through to the `DeviceRGB` default). Terminal-codec payloads stay event-only. New `tests/inline_image_scene_round406.rs`; stencil + Indexed pixel output black-box-validated against Ghostscript
- decode the **full §8.9.5.2 sample model** on the Image-XObject scene splice: `/BitsPerComponent` 1 / 2 / 4 / 8 / 16 (rows byte-aligned, samples MSB-first per §8.9.3), the `/Decode` array with the Table 90 per-space defaults (including the Indexed pass-through and Lab range forms and the NOTE 3 inversion), and every colour space the crate reduces to device RGB — the device families (DeviceCMYK now included), `Indexed`, `ICCBased` via its alternate, the CIE-based `CalGray` / `CalRGB` / `Lab` families, and `Separation` / `DeviceN` through their §7.10 tint transforms — with a *named* image `/ColorSpace` resolved through the page's `/Resources /ColorSpace` subdictionary. Single-component ≤ 8-bit images decode through a precomputed code → RGBA lookup table. New `src/reader/image_decode.rs` + `tests/image_sample_formats_round406.rs`; pixel output black-box-validated against Ghostscript renders of the same fixtures
- paint **§8.9.6 masked images** into the scene: `/ImageMask true` stencil masks (§8.9.6.2 — 1 bit/sample, poured with the *nonstroking colour in force at `Do` time*, `Decode [1 0]` reversal honoured) splice as `Node::Image` with the coverage on the alpha channel; an explicit `/Mask` stencil stream (§8.9.6.3) resamples onto the base image's grid and masks its alpha; a `/Mask` colour-key array (§8.9.6.4) tests each pixel's raw *pre-Decode* codes against the `2 × n` ranges; and `/SMask` **overrides** `/Mask` per Table 89. `ResolvedImageXObject` gains a `stencil` flag
- generalise the image **`/SMask` alpha path** (§11.6.5.3): the soft-mask image now decodes at any supported `/BitsPerComponent` (1 / 2 / 4 / 8 / 16) with its own `/Decode` array honoured, instead of requiring 8-bit tightly-packed gray
- splice **straight-decodable Image XObjects into the `Scene`** on `Do` (§8.9.5 + §8.9.5.2): an image whose `/Filter` chain the crate decodes end-to-end (Flate / LZW / ASCII / RunLength / none) with `/BitsPerComponent 8` and `/DeviceRGB` / `/DeviceGray` becomes a `Node::Image` — the RGBA8 payload combines the colour samples with the `/SMask` soft-mask image's `/DeviceGray` alpha (§11.6.5.3, nearest-neighbour resampled when the mask's dimensions differ per Table 145's independent-size rule), placed on the §8.9.5.2 unit square (sample (0,0) on the top edge) in the writer's own `ImageRef` convention so the node is **writer-round-trippable**: `write_pdf` of an `ImageRef` → `read_pdf_to_scene` reproduces pixels, alpha, and placement exactly. Image-codec payloads (DCTDecode / JPXDecode / …) stay scene-side no-ops on the passthrough walker; a 64 MB decoded-RGBA cap guards memory; gs soft masks wrap spliced images like any other painted object (§11.6.4.2). New `tests/image_scene_round393.rs`
- surface **soft-mask images** on the `image_xobjects()` walker (§11.6.5.3 Tables 145 + 146): `PdfImageXObject` gains `smask: Option<SoftMaskImage>` — the subsidiary `/SMask` image XObject's dimensions, `/BitsPerComponent`, decoded `/DeviceGray` samples (when the filter chain is one the crate decodes; image-codec filters surface `data: None`), and the `/Matte` preblending colour (the parent's samples were premultiplied per `c′ = m + α×(c−m)`). Tolerant: a missing / malformed `/SMask` leaves the field `None` without failing the parent's extraction. New `tests/smask_image_round393.rs`
- writer: emit **`Node::SoftMask` as a real PDF soft mask** (§11.6.4.3 + §11.6.5.2 + §11.6.6): the mask subtree serialises into its own `/G` transparency-group form XObject (own `/Resources`, `/Group << /S /Transparency /CS /DeviceRGB >>` per the Table 147 luminosity-group `/CS` requirement, `/BBox` computed from the subtree's conservative geometry bounds — control points included, arc bulge inflated); an `/ExtGState` entry carries the `/SMask << /Type /Mask /S /Luminosity|/Alpha /G … >>` dictionary mapping `MaskKind::Luminance`/`Alpha`; the content paints inside a `q /GSn gs … Q` bracket so the mask scopes to exactly this composite. A mask subtree with no renderable geometry hides its content entirely (luminosity 0 over the default black backdrop), so writer emits neither the state nor the content. Round-trips through this round's reader paint path (`write_pdf` → `read_pdf_to_scene` reproduces kind + mask geometry + content) and passes `qpdf --check` black-box validation. New `tests/soft_mask_writer_round393.rs`
- composite **transparency-group XObjects** as a unit on `Do` (§11.6.6 Table 147): a form whose `/Group` attributes dictionary has subtype `/S /Transparency` takes the current nonstroking alpha constant on the *spliced group's* opacity — "The nonstroking alpha constant shall also be applied when painting a transparency group's results onto its backdrop" (§11.6.4.4) — while an ordinary form "shall not be subject to any grouping behaviour for transparency purposes" and keeps opacity 1.0. The §11.6.6 initialisation rule (group content starts with blend Normal / alphas 1.0 / soft mask None, "to ensure that they are not applied twice") holds structurally: each form's content stream parses against a fresh graphics state. The active soft mask likewise wraps the group splice once, as a unit. New `tests/transparency_group_round393.rs`
- pour the `/BC` **backdrop colour** under a luminosity soft mask's group (§11.6.5.2 + §11.5.3): the group "shall be composited with a fully opaque backdrop whose colour is everywhere defined by the soft-mask dictionary's BC entry", so a non-black `/BC` lands as the `/G` group's `/BBox` rectangle poured with the colour under the group content (in form space, inside the `/BBox` clip). The colour reduces by component count — 1 gray / 3 RGB / 4 CMYK with the §11.5.3 NOTE 3 device conversions; the default (absent `/BC`, black) inserts nothing since the unpainted mask area already evaluates to zero luminosity. `/TR /Identity` (the Table 144 default) is honoured; a non-identity transfer function has no per-pixel analogue in the vector IR and is tolerated-ignored (documented)
- resolve `/ExtGState /SMask` **soft-mask dictionaries** into the scene (§11.6.4.3 + §11.6.5.2): a `gs` naming a parameter dictionary with an `/SMask` entry establishes the current soft mask; every object painted while it is in force — solid/stroked paths, `Do` form splices, clipped `sh` gradient paints, tiling-pattern fills, Type 3 glyphs — is wrapped in the core IR's `Node::SoftMask`, whose `mask` subtree is the `/G` transparency-group XObject parsed exactly like a `Do`-spliced form (§8.10.1 — `/Matrix` on the transform, `/BBox` as the clip, content against its own `/Resources`, cycle-guarded) and whose `mask_kind` maps `/S /Luminosity` → `MaskKind::Luminance` (§11.5.3) and `/S /Alpha` → `MaskKind::Alpha` (§11.5.2). The mask's coordinate system is fixed at the moment the `gs` executes (§11.6.5.2 — `/Matrix ∘ CTM-at-gs-time`): painting under a later `cm` re-expresses the mask subtree by the inverse of the intervening transform, so the mask stays anchored. `/SMask /None` clears the mask, a `Q` restores the state saved before it was set, and an unresolvable soft-mask dictionary paints unmasked (the standard tolerant degradation). Forms resolve their own `/Resources /ExtGState` soft masks recursively. New entry point `parse_content_stream_full_with_soft_masks` + `ResolvedSoftMask`; new `tests/soft_mask_round393.rs` end-to-end fixtures
...
- gate annotation painting on **optional-content visibility** (`/OC`, §12.5.2 Table 164): "Before the annotation is drawn, its visibility shall be determined based on this entry" — an annotation whose `/OC` resolves invisible "shall be skipped, as if it were not in the document". The catalog's `/OCProperties` is resolved once per decode (§8.11 default-configuration `BaseState` / `ON` / `OFF` algorithm) and each annotation's `/OC` consulted: an optional-content *group* reference paints iff the group's resolved state is ON, an optional-content *membership* dictionary (`/Type /OCMD`) evaluates through its `/P` policy (`AnyOn` / `AllOn` / `AnyOff` / `AllOff`) or `/VE` visibility expression via the existing §8.11 machinery. Tolerant defaults: no `/OC`, an unlayered document (no `/OCProperties`), or an unresolvable entry all paint; an OCG referenced by id but missing from `/OCProperties /OCGs` is hidden. Tests: the same annotation paints with its layer ON and disappears with the layer in the default configuration's `/OFF` list, via both the direct-OCG and OCMD forms
- extend writer-side appearance generation to the **text-markup family** — Highlight / Underline / StrikeOut / Squiggly (§12.5.6.10): each `/QuadPoints` 8-tuple contributes one region (the quad's axis-aligned bounding box — robust to the divergent vertex-order conventions producers emit for Table 179's counterclockwise rule). The spec leaves the markup geometry to the producer, so the writer's convention is documented and stable: a Highlight fills every quad box in one path with the `/C` colour; an Underline strokes a line 10 % above the quad bottom, a StrikeOut at the vertical midpoint (stroke width 7 % of the quad height); a Squiggly strokes a sawtooth along the quad bottom with amplitude 10 % of the height. Degenerate / non-finite quads are skipped; if no quad survives, no `/AP` is emitted. Round-trip tests pin the underline / strike-out line positions, both highlight quads riding one filled path, and the sawtooth's two y-rows
- extend writer-side appearance generation to the **line family** — Line / Ink / Polygon / PolyLine (§12.5.6.7 + §12.5.6.9 + §12.5.6.13): a Line strokes its `/L` endpoints, an Ink strokes each `/InkList` polyline (round caps + joins; single-point strokes skipped), a Polygon closes its `/Vertices` and may pour `/IC` (`f` / `B`), and a PolyLine stays open and stroke-only (its `/IC` colours the undrawn Table 176 line-ending glyphs, so it is not poured). Stroke width comes from the `/Border` array's width component when supplied, else the §12.5.4 "neither `Border` nor `BS` present" default of 1; colour from `/C` (absent → black, zero-component → transparent → no appearance). `/LE` ending glyphs stay undrawn (deferred). Round-trip tests: `/L` endpoints land as a stroked two-point path in the scene, both Ink strokes ride one path, the Polygon closes + fills yellow / strokes blue while the PolyLine stays open and green
- generate writer-side **normal appearance streams** for Square / Circle annotations (§12.5.5 + §12.5.6.8): `write_pdf_with_annotations` now emits a `/Type /XObject /Subtype /Form` appearance stream per geometry-determined annotation and references it from the dict's `/AP << /N … >>`, so conforming readers render the authored appearance instead of a handler-invented one (PDF 2.0 makes `/AP` mandatory for visible annotation types). The form's `/BBox` equals the annotation `/Rect` with an identity `/Matrix`, making the §12.5.5 placement algorithm the identity map — content paints directly in default user space. The rectangle / ellipse is inscribed within `/Rect` (§12.5.6.8) and inset by half the `/BS /W` border width so the border lands completely inside the annotation rectangle (§12.5.4); the ellipse is four cubic quarter-arcs (`4·(√2−1)/3` control offset). `/IC` pours the interior (`f`), `/C` strokes the border (`S`), both paint `B`; the Table 164 colour arities map to `g`/`rg`/`k` + `G`/`RG`/`K`, `/C` absent defaults to black, and an explicitly transparent zero-component colour suppresses that half. An annotation with nothing to paint (no interior, zero-width or transparent border) emits no `/AP` at all. New `tests/annotation_ap_writer_round386.rs` — writer-authored appearances round-trip through the new reader §12.5.5 paint path into the `Scene` (fill+stroke colours, half-width rect inset, four-arc ellipse geometry, outline-only / fill-only modes, empty-paint `/AP` omission)
- surface the appearance dictionary on the `annotations()` extraction walker: `PdfAnnotation` gains `appearance: Option<AnnotationAppearance>` (which of the Table 168 `/N` normal / `/R` rollover / `/D` down entries are present, plus the sorted de-duplicated union of appearance-state names across any subdictionary-form entries) and `appearance_state: Option<String>` (the annotation's `/AS` selector, Table 164). Callers enumerating a form-heavy document can now tell which widgets carry authored appearances and which states (`On` / `Off` / …) each defines without re-walking the raw dictionaries
- honour the display-suppressing **annotation flags** on the appearance-paint path (§12.5.3 Table 165): a **Hidden** (bit 2) annotation "shall not be displayed or printed … regardless of its annotation type", and a **NoView** (bit 6) annotation is hidden for on-screen display — neither reaches the scene, while Print (bit 3) and the interaction-only flags (ReadOnly / Locked / …) leave painting unaffected. A **pop-up annotation** (`/Subtype /Popup`, §12.5.6.14) "shall have no appearance stream … of its own" — its text belongs to the pop-up window machinery, so the subtype is skipped outright even when a producer attaches an `/AP` anyway. Tests: `/F 2` and `/F 32` suppress the appearance, `/F 4` paints; a Popup carrying an `/AP` stays page-invisible
- select the applicable appearance stream by **appearance state** (`/AS`, §12.5.5 + Table 164): an appearance-dictionary entry may be a *subdictionary* of appearance streams keyed by state name (the checkbox `/N << /On … /Off … >>` shape) instead of a single stream; the reader now resolves the annotation's `/AS` name against that subdictionary and paints the selected stream. Table 164 makes `/AS` required whenever `/AP` contains a subdictionary, so an absent `/AS` — or an `/AS` designating a state the subdictionary doesn't define — paints nothing per NOTE 3's "reasonable behaviour". Both the indirect-reference and (tolerated) inline-stream subdictionary entry forms resolve. Tests: `/AS /On` vs `/AS /Off` select the right stream from a two-state Widget; missing / undefined states paint nothing
- paint **annotation appearance streams** into the reader's `Scene` (§12.5.5): every annotation in a page's `/Annots` array (§12.5.2 Table 164) whose `/AP` appearance dictionary carries a normal (`/N`) appearance stream is now rendered on top of the page content instead of being dropped. An appearance stream is a Form XObject (§8.10), so the new document-layer `resolve_annotation_appearances` parses it through the existing `resolve_one_form_xobject` machinery (the form `/Matrix` becomes the inner group transform, `/BBox` the clip, and its content — resolved against its own `/Resources` — the children) and then wraps it in the §12.5.5 *Algorithm: Appearance streams* placement matrix: a) the `/BBox` corners are transformed by `/Matrix` and the smallest upright rectangle enclosing the resulting quadrilateral taken; b) a matrix `A` scales + translates that transformed appearance box onto the annotation `/Rect` (lower-left corner to lower-left corner, upper-right to upper-right, both rectangles normalised per §7.9.5); c) content maps through `AA = Matrix × A`, realised as an outer wrapper group carrying `A` around the inner form group carrying `/Matrix`. A degenerate transformed box (zero width / height, or a non-finite product of a malformed matrix) keeps unit scale on that axis and aligns the lower-left corners only. Annotations paint in `/Annots` array order *after* the page content (§12.5.5 — the appearance composites over "the page content along with any previously painted annotations"), and enumeration is best-effort: a malformed annotation dictionary, a missing `/AP` / `/N` / `/Rect` / `/BBox`, or an undecodable appearance stream contributes nothing rather than aborting the page (NOTE 3's "reasonable behaviour (such as displaying nothing)"). New `tests/annotation_appearance_round386.rs` (a Square annotation's red-square appearance lands inside `/Rect` under the non-uniform §12.5.5 scale and paints after the page content; a rotating `/Matrix` exercises step a's quadrilateral bounding box with the inner group keeping `/Matrix`; no `/AP` and no `/BBox` both paint nothing)
- paint **Type 3 font glyphs** into the reader's `Scene` (§9.6.5): a `Tj` / `TJ` / `'` / `"` show under a Type 3 font now splices each glyph's `/CharProcs` description into the scene tree as vector geometry. Unlike Type 1 / TrueType fonts — whose outlines live in an external font program a software renderer must rasterise — a Type 3 font defines each glyph as a *content stream* of PDF graphics operators, exactly the marking operators this walker already understands. The new document-layer `resolve_type3_fonts` walks the page's (or a Form XObject's / pattern cell's / glyph description's) `/Resources /Font`, and for each `/Subtype /Type3` dictionary reads `/FontMatrix` (default `[0.001 0 0 0.001 0 0]`), parses `/Encoding /Differences` into a code→glyph-name map (§9.6.6.1 — a Type 3 font's entire encoding is carried by `/Differences`), and decodes each `/CharProcs` glyph stream against the font's own `/Resources` into a `Group` — falling back to the enclosing page / Form / pattern-cell resource dictionary when the font omits `/Resources` (§9.6.5 Table 112), so a glyph that paints e.g. a page-level Form XObject still resolves it. At show time the content walker (`paint_type3_show`) resolves each character code to a glyph name and then to a glyph `Group`, and splices it under the glyph's text-rendering matrix — `Tm ∘ [Tfs·Th 0 0 Tfs 0 Trise] ∘ FontMatrix` (§9.4.4 step c) — with the frame's accumulated `cm` applied on pop. The glyph origin advances *between* the bytes of a single show by each glyph's `/Widths` displacement, so a multi-glyph `Tj` lands each glyph in the right place. The `d0` / `d1` glyph-metric operators (§9.6.5 Table 113) are consumed as no-ops (the width is already taken from `/Widths` and the bbox is advisory). The d0 / d1 colour rule is honoured: a **`d1` shape-only glyph** takes the *current fill colour* in force at show time (its description's own colour operators are disregarded per Table 113 + NOTE 2 — the glyph's paths are recoloured to the graphics-state fill at paint time), while a **`d0` self-coloured glyph** keeps the colours baked into its description. Text render mode 3 (invisible — the OCR layer, §9.3.6) suppresses glyph painting, and a glyph name absent from `/Encoding` or `/CharProcs` paints nothing (§9.6.5 step b). Re-entrancy (a glyph description that itself shows Type 3 text) is depth-bounded (8). Threaded into the page, Form-XObject, tiling-pattern-cell, and nested-glyph content paths via the new `parse_content_stream_full_with_type3` entry point. New `tests/type3_glyph_paint_round381.rs` (the §9.6.5 square + triangle example paints two spliced glyph groups whose 750-unit corners land at the page coordinates the text-rendering matrix dictates; `/FontMatrix` scaling enlarges the painted glyph; mode-3 invisible text paints nothing)
- paint a **clipped** axial / radial `sh` shading into the `Scene` (§8.7.4.5): the `sh` operator paints the shading across the current clipping region, so when a clip is in force the reader now fills the active clip path with the equivalent scene gradient (`Paint::LinearGradient` for an axial Type 2 shading, `Paint::RadialGradient` for a radial Type 3) in addition to surfacing the existing `ContentShading` event. The clip path and the shading `Coords` share the current frame's local coordinate basis, so the gradient maps into the clip by identity (the frame's accumulated `cm` is applied once when the node renders) and the painted fill respects the active `ca` alpha. An *unclipped* `sh` would fill the whole page, so it stays event-only (no synthesised page-sized fill); function-based (Type 1) and mesh (Type 4–7) shadings have no single `Paint` analogue and remain event-only. New `tests/sh_clipped_paint_round377.rs` (a `… W n /Sh sh` black→white axial ramp clipped to a rectangle surfaces as a `LinearGradient` fill with the right axis + end-stop colours)
- replicate `/PatternType 1` *tiling* pattern fills (§8.7.3): a `scn`/`SCN` whose `/Pattern` colour-space operand names a tiling pattern in the page's (or a Form XObject's / pattern cell's) `/Resources /Pattern` now tiles the pattern cell across the filled region instead of the conservative black fallback. The new `resolve_tiling_patterns` (document layer) dereferences each `/PatternType 1` *stream*, decodes its cell content stream, parses it against the pattern's own `/Resources` (fonts / ExtGState / shadings / colour spaces / properties / nested Form XObjects / nested tiling patterns) into a cell `Group`, and captures `/BBox`, `/XStep`, `/YStep`, `/Matrix`, and `/PaintType` (Table 75). At fill time the content walker (`emit_tiling_fill`) replicates the cell at integer multiples of `/XStep` / `/YStep` (§8.7.3.1) covering the fill region's bounding box, clips each tile to the cell `/BBox`, and clips the whole tiling to the painted region. The lattice is anchored to the page's default (initial) coordinate space through the pattern `/Matrix` — independent of any `cm` in force at paint time (§8.7.2 NOTE 1) — by emitting the tiled group at the root frame and mapping the fill region back into root-local space; the index range is computed through the inverse of the pattern matrix and the tile count is hard-capped (4096) so a huge fill over a tiny step can't explode. Negative `/XStep` / `/YStep` (allowed by Table 75) and a singular / degenerate matrix are handled (the latter falls back to black). A coloured cell (`/PaintType 1`) paints with its own colours; the cell's solid fills survive into the scene. An **uncoloured** cell (`/PaintType 2`, §8.7.3.3) is a stencil with no colour of its own — it is poured with the underlying colour the `scn`/`SCN` supplies before the pattern name (`c… /Pname`, read by component count: 1 → DeviceGray, 3 → DeviceRGB, 4 → DeviceCMYK), so the same cell shape tiles different regions in different colours; every fill / stroke in the cloned cell is recoloured to that pour colour (default black when no components are given). Threaded into both the page and Form-XObject content paths via the new `parse_content_stream_full_with_tiling` entry point. New end-to-end test `tests/tiling_pattern_round377.rs` (coloured cell replication produces several red tiles with no black region fallback; one clipped group emitted per tile; uncoloured `[/Pattern /DeviceRGB]` cell poured blue)
- consume `BI … ID … EI` inline images inside the content-stream walker (§8.9.7) and surface each as a `ContentInlineImage` event on `ParsedContent::inline_images`. Previously the walker tokenized the raw inline-image payload as if it were operators / numbers: a binary payload could corrupt the surrounding shapes and — when the payload spelled an operator preceded by too few numeric operands — abort the *entire* content-stream parse with an "operator needed N operands" error, so a page carrying one inline image rendered blank. The walker now hands `BI` to the inline-image framer (`reader::inline_images`), which parses the abbreviated dictionary (W/H/CS/BPC/IM/F per Table 92/93), peels wrapping ASCII / Flate / RunLength filters, leaves the terminal codec filter (`/DCT` / `/JPX` / `/JBIG2` / `/CCF`) in place, and locates the framing `EI` per the §8.9.7 whitespace rule. Each event carries the resolved `PdfInlineImage` plus the CTM in force at the `BI` (mapping the unit image square to user space, §8.9.5.1) and the active `W`/`W*` clip path (§8.5.4). A malformed inline-image dictionary is salvaged by skipping to the next `EI` rather than aborting, so trailing shapes survive. Events surface from every `ParsedContent`-returning entry point; the `Group`-returning legacy entries still consume `BI … EI` correctly even though they discard the event. New `tests/inline_image_content_round374.rs` (binary-payload non-corruption, dims + CTM, ASCII-hex peel, stream order, image-mask flag, malformed-dict salvage)
- gradient fills now make the full `write_pdf` → `read_pdf_to_scene` round trip: the writer emits a `Paint::LinearGradient` / `Paint::RadialGradient` fill as a `/PatternType 2` shading pattern, and (as of the shading-pattern read support this round) the reader resolves that pattern back into a scene gradient `Paint`. New `tests/gradient_roundtrip_round369.rs` covers both linear and radial fills surviving the cycle with their axis / radius and end-stop colours preserved
- paint `/PatternType 2` shading-pattern fills (§8.7.3.3 + §8.7.4.5): a `scn`/`SCN` whose `/Pattern` colour-space operand names a shading pattern in the page's `/Resources /Pattern` now paints the equivalent scene gradient instead of falling back to black. An axial shading maps to a `Paint::LinearGradient` (axis endpoints transformed into device space through the pattern `/Matrix` composed with the CTM in effect), a radial shading to a `Paint::RadialGradient` (the ending circle becomes the outer circle, the starting circle the focal point, radii scaled by the matrix). The pattern's `/Shading` is evaluated through the same axial / radial machinery the `sh` operator uses and its colour stops reduce through the shading `ColorSpace` (device / CIE-based / Separation / DeviceN / Indexed). Tiling patterns (`/PatternType 1`) and function-based (Type 1) shadings have no single scene-gradient analogue and keep the conservative black fallback. New `resolve_pattern_resources` (document layer) dereferences `/Resources /Pattern` and prepares each shading pattern's `/Function`; threaded into both the page and Form-XObject content paths via `parse_content_stream_full_with_patterns`. New end-to-end test `tests/shading_pattern_round369.rs`
- resolve a shading's `/ColorSpace` given as a *named* `/Resources /ColorSpace` key (§8.7.4.5.2): an axial / radial / function-based (`evaluate_gradient_shading`) or mesh (`evaluate_mesh_shading`) shading whose `/ColorSpace` entry is a bare name referring to the page's colour-space resource subdictionary (rather than an inline array) now resolves that name through the same path `cs`/`CS` uses, so its stops / vertices sample in the intended space instead of collapsing to `Unknown` and dropping the gradient. Inline-array colour spaces and bare device names are unaffected. The page's resolved `/Resources /ColorSpace` is threaded from the `sh` operator into both shading evaluators
- interpret an `/Indexed` colour table whose base is a CIE-based space (CalGray / CalRGB / Lab, §8.6.6.3): the per-entry table bytes are decoded `0..255 → 0.0..1.0` and mapped into each base component's own range before the CIE → RGB transform — for a Lab base the L* axis spans 0..100 and a*/b* span the space's `/Range`. `/Indexed` previously accepted only device-family bases. New end-to-end document test (`tests/cie_color_spaces_round369.rs`) reads a single-page PDF whose `/Resources /ColorSpace` carries inline CalGray / CalRGB / Lab spaces and verifies the painted fills
- accept a CIE-based alternate space for `/Separation` (§8.6.6.4) and `/DeviceN` (§8.6.6.5): the alternate colour space a tint transform maps into may now be a `CalGray` / `CalRGB` / `Lab` space, not just a device family. The tint-transform output (1 value for a CalGray alternate, 3 for CalRGB / Lab) is rendered to RGB through the CIE-based colour science added this round; for a Lab alternate the three values are interpreted as the L*a*b* triple the tint transform is responsible for emitting in range. `/DeviceN` validates the tint transform's output arity against the alternate's component count (1 or 3) at resolve time — a mismatch keeps the conservative black fallback, as does an `Indexed` / `Separation` / `DeviceN` alternate (forbidden by the spec anyway). Previously such a space collapsed to `Unknown` and painted black
- evaluate CIE-based colour spaces (CalGray §8.6.5.2, CalRGB §8.6.5.3, Lab §8.6.5.4): a `cs`/`CS` naming a `/Resources /ColorSpace` key that resolves to a `[/CalGray …]`, `[/CalRGB …]`, or `[/Lab …]` array is now tracked as a renderable space instead of collapsing to the conservative black fallback. CalGray decodes its single component by `/Gamma` and scales by the `/WhitePoint` to a CIE 1931 XYZ value; CalRGB decodes its three components by the per-channel `/Gamma` and multiplies by the 3×3 `/Matrix`; Lab maps the L*a*b* triple (L* in 0..100, a*/b* clamped into `/Range`) through the implicit two-stage `g(x)` reverse-companding transform. The resulting XYZ is reduced to device RGB through the standard sRGB display colorimetry (IEC 61966-2-1: a fixed XYZ→linear-RGB matrix followed by the sRGB transfer encoding), with out-of-gamut components clamped per §8.6.5's component-range rule. `/WhitePoint` is validated (XW/ZW positive, YW = 1.0) and a non-conforming dictionary keeps the black fallback. All four `sc`/`scn` call sites honour the new spaces — direct fills, the `cs`/`CS` initial-colour (components initialised to 0.0 per §8.6.5), mesh-shading vertex/corner colours, and an `/Indexed` base whose colour table is interpreted in a CIE-based space (the per-entry bytes scaled into each base component's range, including Lab's L*/Range axes). Component arities: CalGray 1, CalRGB / Lab 3
- inherit `MediaBox`, `Resources`, and `Rotate` through the page tree (§7.7.3.4): a leaf page that omits an inheritable attribute now takes the value from the nearest ancestor `/Pages` node by walking the `/Parent` chain (`resolve_inheritable_attr`), instead of defaulting `MediaBox` to A4 and treating `Resources` as empty. Documents that hang one resource dictionary (fonts / XObjects / shadings / colour spaces) or one media box on an intermediate page-tree node — a common authoring convention — now resolve their content correctly rather than rendering blank or mis-sized. The walk is depth-bounded (64) and cycle-guarded against a malformed self-referential `/Parent`
- read the inheritable page `/Rotate` (§7.7.3.3 Table 30) onto `Page::orientation`: the page's clockwise rotation in degrees (a multiple of 90, default 0) is resolved through the same `/Parent` inheritance walk and normalised into the canonical `0` / `90` / `180` / `270` the scene `Page` carries (negative and `>360` multiples of 90 fold via `rem_euclid`, a leaf override wins over the parent, and a non-multiple of 90 is treated as malformed and left at 0). Previously the reader always reported orientation 0. `CropBox` remains inheritable per Table 30 but is not yet consumed
- paint Form XObjects on the `Do` operator into the reader's `Scene` (§8.10): a `name Do` that resolves to a `/Resources /XObject` entry whose `/Subtype` is `/Form` now splices the form's content into the scene tree instead of dropping it. Each page's `/XObject` subdictionary is resolved (`resolve_xobject_forms`) into a map of pre-parsed form `Group`s: the form's content stream is decoded, its own `/Resources` (fonts, ExtGState, shadings, colour spaces, properties, *and* nested Form XObjects) are resolved, and its content is recursively parsed. The resulting `Group` carries the form's `/Matrix` as its `transform` and the `/BBox` rectangle as its `clip`, so `Do` reproduces the §8.10.1 algorithm (q / concat-Matrix-with-CTM / clip-to-BBox / paint / Q) by splicing the group under the current frame's accumulated CTM. Form recursion is depth-bounded (12) and cycle-guarded by the in-flight form object-id set, so a self-referential or mutually-recursive appearance stream terminates and surfaces its own painted content once. Image XObjects stay a vector-side no-op (they are surfaced separately by `image_xobjects()`). New entry point `parse_content_stream_full_with_xobjects`
- scale Type 3 font advances by `/FontMatrix`, not 1/1000 (§9.6.5 / §9.2.4): a Type 3 font's `/Widths` are expressed in glyph space and converted to text space by the horizontal component of the font's `/FontMatrix`, unlike Type1 / TrueType widths which are in thousandths of text space. Both the content-walker advance (`reader::content`) and the public `text_extraction()` surface (`reader::text`) now carry a per-font `text_scale` derived from the `/FontMatrix` for Type 3 fonts (default `0.001` for the common `[0.001 0 0 0.001 0 0]` matrix, and for all other simple/composite fonts), so a Type 3 font with a non-default matrix (e.g. `[0.01 …]`) reports correctly-spaced text-run origins instead of advances 10× too small. Composite (`/W` / `/DW`) and Type1 / TrueType behaviour is unchanged
- evaluate Order-3 (cubic-spline) Type 0 (sampled) functions (§7.10.2): a `/Order 3` sampled function is now interpolated with a per-axis cubic spline that passes through the four nearest sample knots (Catmull-Rom cardinal basis; the weights sum to 1 at every fractional position, so the interpolant reproduces each sample exactly at integer encoded positions and a constant table stays flat). The tensor-product blend generalises across any number of input dimensions, replacing the prior Order-1-only multilinear path with a unified per-axis tap accumulator. Per §7.10.2, an input axis whose `/Size` is below 4 cannot carry a cubic window and falls back to linear interpolation on that axis; an axis at the table edge clamps its window onto the boundary sample (no extrapolation). `/Order` values other than 1 or 3 are rejected as malformed (Table 39: "Valid values shall be 1 and 3"). Order-1 remains the default and its midpoint behaviour is unchanged. This applies wherever a Type 0 function is evaluated — `/Separation` and `/DeviceN` tint transforms and shading colour functions
- advance the extracted text origin between text-showing operators in `text_extraction()` (§9.4.3 / §9.4.4): the extraction walker now applies the same per-glyph displacement, so a `TextRun` shown after a prior `Tj` / `TJ` / `'` / `"` on the *same* line (no intervening `Td` / `Tm`) reports its true origin instead of the previous run's. Per-glyph advance widths come from the font's `/Widths` (simple) or descendant CIDFont `/W` / `/DW` (Type0 Identity) — indirect arrays are dereferenced at page load. `Tc` / `Tw` / `Tz` are tracked and `"` applies its `aw ac` operands; `TJ` kerns shift the origin by `−adj/1000·Tfs·Th`. Saved / restored by `q` / `Q`. Fonts with no width data advance by 0
- advance the text matrix between text-showing operators (§9.4.3 / §9.4.4): consecutive `Tj` / `TJ` / `'` / `"` shows on one line now report distinct origins. After each glyph the text matrix is translated by `tx = ((w0 − Tj/1000)·Tfs + Tc + Tw)·Th`, with per-glyph advance widths `w0` resolved from the font's `/Widths` array (simple fonts, `/FirstChar` + `/MissingWidth` from the `/FontDescriptor`) or the descendant CIDFont's `/W` / `/DW` (Type0 Identity, §9.7.4.3). The `Tc` (char-spacing), `Tw` (word-spacing, single-byte code 32 only) and `Tz` (horizontal-scaling) text-state operators are now tracked and fed into the displacement; `"` applies its leading `aw ac` operands as `Tw` / `Tc`. `TJ` per-element numeric kerns adjust the origin by `−adj/1000·Tfs·Th` (§9.4.3). Fonts with no resolvable width data advance by 0, preserving prior behaviour
- evaluate Type 1–3 (function-based / axial / radial) shadings (§8.7.4.5.2–4): a `name sh` paint now surfaces sampled gradient geometry on the new `ContentShading::gradient` field. An axial (Type 2) shading carries its `[x0 y0 x1 y1]` axis + `Extend` flags + 64 RGB colour stops sampled uniformly across the parametric `Domain`; a radial (Type 3) shading carries its `[x0 y0 r0 x1 y1 r1]` circles + stops; a function-based (Type 1) shading carries its `[xmin xmax ymin ymax]` domain rectangle + `Matrix` + a 16×16 RGB grid of its 2-in/n-out colour function. Each stop / sample is reduced to device RGB through the shading's `ColorSpace` and `/Function` (Type 0/2/3/4, single or array). `gradient` and `mesh` are mutually exclusive
- evaluate Type 4–7 (mesh) shadings (§8.7.4.5.5–8.7.4.5.8): a `name sh` paint of a free-form (Type 4) / lattice-form (Type 5) Gouraud-shaded triangle mesh, a Coons patch mesh (Type 6), or a tensor-product patch mesh (Type 7) is decoded from its bit-packed stream body into device-space geometry on the `ContentShading::mesh` field. The stream payload (vertex coordinates + edge flags + colour data) is unpacked MSB-first at `BitsPerCoordinate` / `BitsPerComponent` / `BitsPerFlag` widths, decoded through the `Decode` array's §8.9.5.2 linear map, and each vertex / corner colour reduced to RGB through the shading's `ColorSpace` (device / Indexed / Separation / DeviceN) and, when present, its parametric `/Function` (a single value `t` per vertex mapped by a 1-in/n-out function or n 1-in/1-out functions). Type 4 edge flags (`f=0` new triangle, `f=1`/`2` continuation per §8.7.4.5.5 Figure 25) and Type 6/7 patch edge flags (`f=0..3`, with shared boundary points + corner colours inherited from the previous patch per Tables 85/86) are honoured; Coons patches are expanded to the equivalent 16-control-point tensor surface via the §8.7.4.5.8 internal-control-point equations. The decompressed mesh body is folded into the resolved shading dictionary under `__MeshData` and its `/Function` entry prepared in place (`resolve_shading_resources`), mirroring the Type 0 `__Samples` handling
